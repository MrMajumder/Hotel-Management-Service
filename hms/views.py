...
def getbilltable(resid):
    cursor = connection.cursor()
    sql = ("SELECT C.RESERVATION_ID, C.ARRIVAL_DATE, C.DEPARTURE_DATE, D.FNAME, D.CNO, D.PNO, B.BILL_ID, B.COST, B.BILL_DATE, B.VAT_PERCENTAGE, A.DUE, E.ROOM_ID, E.ROOM_TYPE, E.CAPACITY, E.RENT, C.RESERVATION_ACTIVE FROM HOTEL_BILL A, BILL B, RESERVATION C, (SELECT X.USER_ID USID, (Y.FIRST_NAME || ' ' || Y.LAST_NAME) FNAME, X.ID_CARD_NO CNO, ((SELECT PH_NUMBER FROM ACCOUNT_HOLDER_PHNUMBER WHERE ACCOUNT_HOLDER_PHNUMBER.USER_ID = X.USER_ID AND ROWNUM <= 1)) PNO FROM CUSTOMER X, ACCOUNT_HOLDER Y WHERE X.USER_ID = Y.USER_ID) D, ROOM E, BOOKED_ROOMS F WHERE A.BILL_ID = B.BILL_ID AND A.RESERVATION_ID = C.RESERVATION_ID AND D.USID = C.USER_ID AND C.RESERVATION_ID = F.RESERVATION_ID AND F.ROOM_ID = E.ROOM_ID AND C.RESERVATION_ID = %s" % (resid))
    cursor.execute(sql)
    row = cursor.fetchall()
